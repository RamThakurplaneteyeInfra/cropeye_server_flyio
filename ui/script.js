document.addEventListener("DOMContentLoaded", function () {
  ...
  sendOtpBtn.addEventListener("click", function () {
    ...
    fetch(API_LOGIN_URL, {
        ...
        if (!response.ok) {
          return response
            ...
            .then((errorData) => {
              ...
              throw new Error(
                `Authentication failed: ${errorData.detail || errorData.error || "Unknown error"}`,
              );
            })
            .catch((e) => {
      ...
      .then((userData) => {
        ...
        if (!userEmail) {
          throw new Error(
            "No email address found for this user. Please contact administrator.",
          );
        }
      ...
      .then((data) => {
        ...
        console.log(
          "OTP requested successfully and sent to registered email: " +
            userEmail,
        );
      })
      .catch((error) => {
        ...
        if (error.message.includes("Invalid username or password")) {
          ...
          alert(
            "No email address registered for this account. Please contact your administrator.",
          );
  ...
  document
    .getElementById("create-booking-btn")
    .addEventListener("click", function () {
      hideAllSections();
      bookingsSection.classList.remove("hidden");
      navLinks.forEach((l) => l.classList.remove("active"));
  ...
  if (addVendorBtn) {
    addVendorBtn.addEventListener("click", function () {
      ...
      document.querySelector("#vendor-modal .btn.primary-btn").textContent =
        "Add Vendor";
    });
  }
    ...
    try {
      ...
      if (currentUserData && currentUserData.roles) {
        ...
        try {
          ...
          console.log(
            "Roles endpoint not available - user_role field will be optional",
          );
        }
      }
    } catch (error) {
  ...
  orderItemsContainer.addEventListener("click", function (e) {
    if (e.target.classList.contains("remove-item-btn")) {
      ...
      if (
        itemRow &&
        orderItemsContainer.querySelectorAll(".order-item-row").length > 1
      ) {
        itemRow.remove();
        updateRemoveButtons();
      }
      ...
      itemRows.forEach((row) => {
        ...
        if (itemName && itemName.trim()) {
          const item = {
            ...
            year_of_make:
              row.querySelector('input[name*="[year_of_make]"]').value || null,
            estimate_cost:
              row.querySelector('input[name*="[estimate_cost]"]').value || null,
        ...
        if (error.errorData) {
          ...
          for (const field in error.errorData) {
            if (Array.isArray(error.errorData[field])) {
              errorDetails.push(
                `${field}: ${error.errorData[field].join(", ")}`,
              );
            } else {
  ...
  document
    .getElementById("manage-equipment-btn")
    .addEventListener("click", function () {
      hideAllSections();
      equipmentSection.classList.remove("hidden");
      navLinks.forEach((l) => l.classList.remove("active"));
  ...
  document.querySelectorAll(".delete-btn").forEach((button) => {
    button.addEventListener("click", function () {
      ...
      if (confirm("Are you sure you want to delete this item?")) {
        alert(
          "Item with ID: " + id + " would be deleted in a real application.",
        );
    ...
    fetch(API_USERS_ME, {
      ...
      .then((userData) => {
        ...
        if (industryId) {
          ...
          showTeamConnectError(
            "No industry assigned. Please contact administrator.",
          );
        }
      })
      .catch((error) => {
  ...
  function showTeamConnectLoading() {
    const loadingHtml =
      '<tr><td colspan="6" style="padding: 20px; text-align: center; color: #757575;">Loading...</td></tr>';
    const loadingHtmlSingle =
      '<tr><td colspan="1" style="padding: 20px; text-align: center; color: #757575;">Loading...</td></tr>';
  ...
  function renderAllTeamConnectTables() {
    ...
    if (teamConnectData.counts) {
      const fieldOfficerCountEl = document.getElementById(
        "field-officer-count",
      );
      ...
      if (fieldOfficerCountEl)
        fieldOfficerCountEl.textContent = `(${teamConnectData.counts.field_officers_count || 0})`;
      if (ownerCountEl)
        ownerCountEl.textContent = `(${teamConnectData.counts.owners_count || 0})`;
      if (managerCountEl)
        managerCountEl.textContent = `(${teamConnectData.counts.managers_count || 0})`;
      if (farmerCountEl)
        farmerCountEl.textContent = `(${teamConnectData.counts.farmers_count || 0})`;
      if (bookingCountEl)
        bookingCountEl.textContent = `(${teamConnectData.counts.bookings_count || 0})`;
      if (orderCountEl)
        orderCountEl.textContent = `(${teamConnectData.counts.orders_count || 0})`;
      if (stockItemCountEl)
        stockItemCountEl.textContent = `(${teamConnectData.counts.stock_items_count || 0})`;
      if (vendorCountEl)
        vendorCountEl.textContent = `(${teamConnectData.counts.vendors_count || 0})`;
    }

    // Render each table separately
    renderTeamConnectTable(
      "field_officers",
      "field-officer-tbody",
      teamConnectData.users_by_role.field_officers || [],
    );
    renderTeamConnectTable(
      "owners",
      "owner-tbody",
      teamConnectData.users_by_role.owners || [],
    );
    renderTeamConnectTable(
      "managers",
      "manager-tbody",
      teamConnectData.users_by_role.managers || [],
    );
    renderTeamConnectTable(
      "farmers",
      "farmer-tbody",
      teamConnectData.users_by_role.farmers || [],
    );

    // Render count tables for Booking, Orders, Stock Items, and Vendors
    renderCountTable(
      "booking-tbody",
      teamConnectData.counts?.bookings_count || 0,
    );
    renderCountTable("order-tbody", teamConnectData.counts?.orders_count || 0);
    renderCountTable(
      "stock-item-tbody",
      teamConnectData.counts?.stock_items_count || 0,
    );
    renderCountTable(
      "vendor-tbody",
      teamConnectData.counts?.vendors_count || 0,
    );
  }
  ...
  function renderTeamConnectTable(roleType, tbodyId, users) {
    ...
    if (users.length === 0) {
      tbody.innerHTML =
        '<tr><td colspan="6" style="padding: 20px; text-align: center; color: #757575;">No users found for this role.</td></tr>';
      return;
    }
  ...
  async function loadBookings() {
    ...
    tbody.innerHTML =
      '<tr><td colspan="7" style="padding: 20px; text-align: center; color: #757575;">Loading...</td></tr>';
  ...
  function displayBookings(bookings) {
    ...
    if (bookings.length === 0) {
      tbody.innerHTML =
        '<tr><td colspan="7" style="padding: 20px; text-align: center; color: #757575;">No bookings found.</td></tr>';
      return;
    }
  ...
  async function loadVendors() {
    ...
    tbody.innerHTML =
      '<tr><td colspan="8" style="padding: 20px; text-align: center; color: #757575;">Loading...</td></tr>';
  ...
  function displayVendors(vendors) {
    ...
    if (vendors.length === 0) {
      tbody.innerHTML =
        '<tr><td colspan="8" style="padding: 20px; text-align: center; color: #757575;">No vendors found.</td></tr>';
      return;
    }
  ...
  function displayOrders(orders) {
    ...
    if (orders.length === 0) {
      tbody.innerHTML =
        '<tr><td colspan="7" style="padding: 20px; text-align: center; color: #757575;">No orders found.</td></tr>';
      return;
    }
  ...
  function displayStockItems(stockItems) {
    ...
    if (stockItems.length === 0) {
      tbody.innerHTML =
        '<tr><td colspan="8" style="padding: 20px; text-align: center; color: #757575;">No stock items found.</td></tr>';
      return;
    }
        ...
        if (searchValue.length > 2 || searchValue.length === 0) {
          try {
            const url = searchValue
              ? `${API_VENDORS}?search=${encodeURIComponent(searchValue)}`
              : API_VENDORS;
    ...
    orderSearchInput.addEventListener(
      ...
      debounce(async function () {
        ...
        if (searchValue.length > 2 || searchValue.length === 0) {
          try {
            const url = searchValue
              ? `${API_ORDERS}?search=${encodeURIComponent(searchValue)}`
              : API_ORDERS;
            const data = await apiCall(url);
    ...
    stockSearchInput.addEventListener(
      ...
      debounce(async function () {
        ...
        if (searchValue.length > 2 || searchValue.length === 0) {
          try {
            const url = searchValue
              ? `${API_STOCK}?search=${encodeURIComponent(searchValue)}`
              : API_STOCK;
            const data = await apiCall(url);
  ...
  window.editVendor = async function (id) {
    try {
      ...
      document.getElementById("vendor-contact-person").value =
